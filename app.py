...
def filter_catalog():
    try:
        if 'pdf' not in request.files:
            return jsonify({"error": "Файл не передан (ожидается поле 'pdf')"}), 400

        numbers = (request.form.get('numbers') or '').strip()
        if not numbers:
            return jsonify({"error": "Не переданы номера позиций (поле 'numbers')"}), 400
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            logger.info(f"Processing catalog, numbers='{numbers}'")
